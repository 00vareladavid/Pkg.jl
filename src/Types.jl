...
function remote_dev_path!(ctx::Context, pkg::PackageSpec, shared::Bool)
    ...
    set_repo_for_pkg!(ctx.env, pkg)
    repo_path = fresh_clone(pkg)
    # parse repo to determine dev path
    parse_package!(ctx, pkg, repo_path)
    canonical_dev_path!(ctx, pkg, shared; default=repo_path)
    return pkg.uuid
end
