...
# READING #
###########
# TODO we don't expected to read `pinned == false`, do anything about that case?
read_pin(pinned::Bool) = pinned
read_pin(pinned) = pkgerror("Expected field `pinned` to be a Bool")
...
function Manifest(raw::Dict)::Manifest
    ...
    for (name, infos) in raw, info in infos
        ...
        entry = PackageEntry()
        entry.name         = name
        entry.pinned       = read_pin(get(info, "pinned", false))
        uuid               = read_field("uuid",          nothing, info, safe_uuid)
        entry.git_tree_sha = read_field("git-tree-sha1", nothing, info, safe_SHA1)
        entry.version      = read_field("version",       nothing, info, safe_version)
        entry.path         = read_field("path",          nothing, info, identity)
        entry.repo_url     = read_field("repo-url",      nothing, info, identity)
        entry.repo_rev     = read_field("repo-rev",      nothing, info, identity)
