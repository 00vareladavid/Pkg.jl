...
@testset "Depot setup" begin
    isolate() do
        ...
        Pkg.dependencies(exuuid) do pkg
            ...
            @test isdir(pkg.source)
            @test isdir(pkg.git_cache)
        end
        Pkg.add(Pkg.PackageSpec(;url="https://github.com/JuliaLang/Example.jl.git", rev="v0.5.3"))
        Pkg.dependencies(exuuid) do pkg
            @test !pkg.is_tracking_registry
            @test isdir(pkg.source)
            @test isdir(pkg.git_cache)
...
# No harm in using the loaded depot here.
@testset "add: input checking" begin
    isolate(loaded_depot=true) do
        ...
        # Wrong UUID
        @test_throws PkgError("expected package `Example [00000000]` to be registered"
            ) Pkg.add(Pkg.PackageSpec("Example", UUID(UInt128(1))))
        # Missing UUID
...
@testset "add: changes to the active project" begin
    ...
    isolate(loaded_depot=true) do; mktempdir() do tempdir
        ...
        Pkg.dependencies(simple_package_uuid) do pkg
            @test realpath(pkg.git_source) == realpath(path) #TODO
...
@testset "add: repo handling" begin
    ...
    isolate(loaded_depot=true) do; mktempdir() do tempdir
        ...
        path = abspath(path)
        Pkg.add(Pkg.PackageSpec(;path=path))
        # We check that the path is stored in an absolute format.
        Pkg.dependencies(empty_package) do pkg
            @test isabspath(pkg.git_source)
        end
    ...
    isolate(loaded_depot=true) do; mktempdir() do tempdir
        ...
        # We check that the path is stored in a relative format.
        Pkg.dependencies(empty_package) do pkg
            @test !isabspath(pkg.git_source)
        end
        # Now we try to find the package.
...
@testset "develop: input checking" begin
    isolate(loaded_depot=true) do
        ...
        # Wrong UUID
        @test_throws PkgError("expected package `Example [00000000]` to be registered"
            ) Pkg.develop(Pkg.PackageSpec("Example", UUID(UInt128(1))))
        # Missing UUID
...
@testset "develop: path handling" begin
    ...
    isolate(loaded_depot=true) do
        ...
        mktempdir() do tempdir
            ...
            Pkg.dependencies(simple_package_uuid) do pkg
                @test isdir(pkg.source)
                @test pkg.source == joinpath(tempdir, "SimplePackage")
            end
        end
    end
    # Absolute paths
